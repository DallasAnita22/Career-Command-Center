                ...
                if not st.session_state['gemini_key']: st.error("Please enter API Key in Sidebar")
                    ...
                    try:
                        with st.spinner("Coach is reading..."):
                            ...
                            if ai.client:
                                response = ai.client.models.generate_content(model='gemini-flash-latest', contents=prompt)
                ...
                with st.expander("🤖 View AI Verdict (Gemini)", expanded=False):
                    if st.button("Ask AI for 'The Fix'"):
                        ...
                        if ai.client:
                            ...
                            resp = ai.client.models.generate_content(model='gemini-flash-latest', contents=prompt)
                            st.write(resp.text)
